...
exports.eachSubModel = (dataType, models, opts) => {
  const modelObj = models.find(m => dataType === m.name)
  if (modelObj) {
    console.log('model: ' + modelObj.name)
    return modelObj.fields.map(field => {
      console.log('field: ' + field.name)
      const subModelObj = models.find(m => field.type === m.name || field.type === '[' + m.name + ']')
      if (subModelObj) {
        subModelObj._meta_ = {
          fieldName: field.name,
          isArray: (field.type.replace(/^\[/, '').replace(/\]$/, '') !== field.type)
        }
